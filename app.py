...
</style></head><body><main class="wrap"><header><div><div class="eyebrow">Local-first · Bordeaux / remote</div><h1>Bordeaux Job<br>Cockpit</h1><p class="intro">A calm, actionable view of Bordeaux-area AI, robotics and product opportunities — with a concrete next move for every lead.</p></div><div class="privacy">↗ No account · no tracking · your browser</div></header>
...
<section id="grid" class="grid">{cards}</section><footer>Sample data bundled with this project. It reads only its own copy in <code>data/jobs.json</code>. Click ✓ to hide a lead locally; reset with <code>localStorage.clear()</code>.</footer></main>
<script>
...
# Source note: sample records are a separate, user-editable project data copy.
# The app reads only its own bundled data file.
