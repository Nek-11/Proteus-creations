...
A small, local-first dashboard for turning Bordeaux-area AI job research into a next-action pipeline.
...
This is intentionally self-contained and only reads its bundled `data/jobs.json` file. Edit that file when you want to add or revise leads.
...
The artifact is reversible. If no longer useful, stop the server and optionally remove only this directory:
...
`data/jobs.json` contains demo/sample opportunity data bundled with the project.
